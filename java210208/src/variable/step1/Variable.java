...
public class Variable {
	...
	double methodB(int x) {//x는지변 밖에서는 사용불가
	...
	public static void main(String[] args) {
		//System:너의 컴터 out속성:출력장치(하드웨어-모니터, 프린터....) , print():이건 illegal, print(int i), print(double pi), print("안녕")
		System.out.print(5);//줄바꿈 안됨, 메소드를 호출할 수 있니? 너는????? 주소번지.methodA();
		//메소드 선언할 때는 좌중괄호 우중괄호 를 사용하고...
		//메소드 호출할때는 ;
		//API, XXX.jar, 이 안(클래스[소스가 만들어졌으니까 구체적이다]안에 객체[결정되지 않았다.Sonata.java, class Sonata{}] 안에)에 있는 것을 꺼내쓴다.
		//print메소드는 여러개이다.같은 이름의 메소드를 여러개 만들 수도 있다.
		//SELECT 이름 FROM 회원 집합
		// WHERE 아이디='test'
		//   AND 비번='123'
		//System.out.print();//줄바꿈 안됨- 반환값이 없으니까 에러인거야? 출력결과 물
